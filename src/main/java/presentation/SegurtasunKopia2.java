...
public class SegurtasunKopia2 extends JFrame {
	...
	private final JComboBox flightListJComboBox = new JComboBox();
	private final JComboBox comboBox = new JComboBox();
	private final JComboBox selectFlight = new JComboBox();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SegurtasunKopia2 frame = new SegurtasunKopia2();
	...
	private SegurtasunKopia2() {
		...
		// USTE DET FLIGHTINFO_Cn dagoela errorea hori dela eta emanten du hau zeren eta
		// flighinfo_c string bat da eta bilatzen duguna da concreteflight motako modelo
		// bat jarraitzea. Hori dela eta aldaketa egin beharko da atal horretan
		...
		flightList.addListSelectionListener(new javax.swing.event.ListSelectionListener() {
			public void valueChanged(javax.swing.event.ListSelectionEvent e) {
				...
				if (!flightList.isSelectionEmpty()) {
					...
					bookFlight.setText("Book: " + selectedConcreteFlight); // TODO Auto-generated Event stub
																			// valueChanged()
				}
			}
		});
		...
		lookforFlights.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				System.out.println(concreteFlightCollection.toString());
				Iterator<ConcreteFlight> flights = concreteFlightCollection.iterator();
				while (flights.hasNext()) {
					System.out.println("PASS");
					selectedConcreteFlight = (ConcreteFlight) flightList.getSelectedValue();
					System.out.println("PASS");
					flightInfo_c.addElement(flights.toString());
		...
		jLabelResult.setBounds(117, 178, 243, 16);
		...
		// COMBOBOX HIRIEN IZENEKIN
		dCities = new JComboBox();
		dCities.setModel(new DefaultComboBoxModel(
				new String[] { "Barcelona ", "Donostia", "Sevilla", "Malaga", "Santander", "Granada" }));
		dCities.setBounds(99, 9, 243, 16);
		contentPane.add(dCities);

		aCities = new JComboBox();
		aCities.setModel(
				new DefaultComboBoxModel(new String[] { "Bilbo", "Madrid", "Malaga", "Santander", "Granada" }));
		aCities.setBounds(95, 31, 247, 21);
		contentPane.add(aCities);

		// HIRIAK SELEKZIONATZEKO
		selectFlight.setModel(new DefaultComboBoxModel(new String[] { "flightInfo.toString()" }));
		selectFlight.setBounds(31, 156, 379, 34);
		contentPane.add(selectFlight);
		selectFlight.setModel(flightInfo_c);
	}
